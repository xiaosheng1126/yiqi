...
public class DensityUtils
{
    ...
    public static int getScreenWidthToDp(Context context)
    {
        test(context);
        DisplayMetrics displayMetrics = new DisplayMetrics();
        ((WindowManager) getSystemService(context, Context.WINDOW_SERVICE)).getDefaultDisplay().getMetrics(displayMetrics);
        return (int) (displayMetrics.widthPixels * 1.0  / displayMetrics.density);
    }
    ...
    private static Object getSystemService(@NonNull Context context, @NonNull String name){
        test(context);
