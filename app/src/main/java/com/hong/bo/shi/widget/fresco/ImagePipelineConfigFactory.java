...
public class ImagePipelineConfigFactory {
    ...
    public static ImagePipelineConfig getImagePipelineConfig(Context context) {
        if (sImagePipelineConfig == null)
            sImagePipelineConfig = configureCaches(context, RetrofitHelper.getFrescoOkHttpClient());
    ...
    private static ImagePipelineConfig configureCaches(Context context, OkHttpClient okHttpClient) {
        ...
        DiskCacheConfig diskSmallCacheConfig = DiskCacheConfig.newBuilder(context)
                .setBaseDirectoryPath(App.getInstance().getFresoCache().getAbsoluteFile())//缓存图片基路径
...
//            .setResizeAndRotateEnabledForNetwork(boolean resizeAndRotateEnabledForNetwork)//调整和旋转是否支持网络图片
    ...
    private static class ConfigConstants {
        ...
        public static final int MAX_SMALL_DISK_CACHE_VERY_LOW_SIZE = 20 * ByteConstants.MB;//小图极低磁盘空间缓存的最大值(特性：可将大量的小图放到额外放在另一个磁盘空间防止大图占用磁盘空间而删除了大量的小图)
        public static final int MAX_SMALL_DISK_CACHE_LOW_SIZE = 40 * ByteConstants.MB;//小图低磁盘空间缓存的最大值(特性：可将大量的小图放到额外放在另一个磁盘空间防止大图占用磁盘空间而删除了大量的小图)
        public static final int MAX_SMALL_DISK_CACHE_SIZE = 80 * ByteConstants.MB;//小图磁盘缓存的最大值(特性：可将大量的小图放到额外放在另一个磁盘空间防止大图占用磁盘空间而删除了大量的小图)

        public static final int MAX_DISK_CACHE_VERY_LOW_SIZE = 40 * ByteConstants.MB;//默认图极低磁盘空间缓存的最大值
        public static final int MAX_DISK_CACHE_LOW_SIZE = 120 * ByteConstants.MB;//默认图低磁盘空间缓存的最大值
